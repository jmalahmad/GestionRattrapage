...
<%@page import="tn.iset.model.Registre"%>
...
<%
 ...
 %> <%
 	if (session.getAttribute("tst") != null) {
 		if (!((Registre) session.getAttribute("ut9")).getUsername().equals("admin.tn")) {
 			response.sendRedirect("/PrjIsetWeb/dali/login.jsp");
 		}
			...
			<div class="row-fluid">
				...
				<div class="span6">
					...
					<div class="containerHeadline">
						<i class="icon-list"></i>
						<h2>Limite des absences à affichées :</h2>
					...
					<div class="floatingBox">
						<div class="container-fluid">
							<form class="form-horizontal contentForm"
								action="../GestionAbsence" method="get">
								<input type="hidden" name="action" value="affichAbs" />


								<div class="control-group">
									<label class="control-label">Date de Debut</label>
									<div class="controls">
										<input id="datepickerDateDebut" type="text" class="span10"
											name="dateDebut"
											value="<%=(session.getAttribute("dateDebut") == null) ? laDateDuJour : session.getAttribute("dateDebut")%>">
									</div>
								</div>
								...
								<div class="control-group">
									...
									<div class="controls">
										<input id="datepickerDateFin" type="text" class="span10"
											name="dateFin" value="<%=(session.getAttribute("dateDebut") == null) ? laDateDuJour : session.getAttribute("dateFin")%>">
									</div>
								</div>
								...
								<div class="formFooter">
									<center>
										<button type="submit" class="btn btn-primary">
											<i class="icon-filter"></i>Lister les absences
										</button>
									</center>
				...
				<div class="span6">
					...
					<div class="containerHeadline">
						...
						<h2>
							Absences de la période entre
							<%=(session.getAttribute("dateDebut") == null) ? laDateDuJour : session.getAttribute("dateDebut")%>
							et
							<%=(session.getAttribute("dateFin") == null) ? laDateDuJour : session.getAttribute("dateFin")%></h2>
						<div class="controlButton pull-right">
							<i class="icon-remove removeElement"></i>
						</div>
						<div class="controlButton pull-right">
									...
									<div class="container-fluid">
										...
										<table class="tablesorter centerFirstLast" id="membersTable">
											<thead>
												<tr>
													<th data-placeholder="search in Semestre..."><CENTER>Nom
															Enseignant</CENTER></th>
													<th data-placeholder="search in usernames...">Nom
														Groupes</th>
													<th data-placeholder="search in Semestre...">Nom Salle</th>
													<th data-placeholder="search in usernames...">Nom
														Séance</th>
												</tr>
											</thead>
											<tbody>
												...
												<c:forEach items="${sessionScope.listAbsence}" var="Abs">
													<tr>


														<td><c:out value="${Abs.nomEns}"></c:out></td>
														<td><c:out value="${Abs.nomSalle}"></c:out></td>
														<td><c:out value="${Abs.nomGroupe}"></c:out></td>

														<td><c:out value="${Abs.nomSeance}"></c:out></td>
